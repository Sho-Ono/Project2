...
/**
 * Class to represent a collection of Tweets.
 * 
...
public class TweetCollection
{
	...
	public String getNthTweet() {
		return nthTweet.getText();
	}
	
public void setNthTweet(int n) {
	BufferedReader lineReader = null;
	try {
		lineReader = new BufferedReader(new FileReader(filename));
		String line = null;
		...
		while ((line = lineReader.readLine()) != null) {
			...
			try {
				...
				tweets.put(id,
						new Tweet(polarity, id, tokens[2], tokens[3]));
				if (lineNum == n) {
					nthTweet = new Tweet(polarity, id, tokens[2], tokens[3]);
				}
			} catch (Exception e) {
				System.err.println("Format error at line " + lineNum + ".");
			}
	 ...
	 * 
	 * @param t The tweet to predict the polarity of.
	...
	public int predict(Tweet t)
	{
		...
		int prediction;
		prediction = (generator.nextInt(3) * 2);
